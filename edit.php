...
// ・投稿したつぶやき
// ・投稿した日時
    $sql = 'SELECT `tweets`.`tweet`,`members`.`nick_name`,`members`.`picture_path`,`tweets`.* FROM `tweets` INNER JOIN `members` on `tweets`.`member_id` = `members`.`member_id` WHERE `tweet_id`='.$_REQUEST['tweet_id'];
        ...
        <div class="msg">
          ...
          <p>
            ...
            <form method="post" action="" class="form-horizontal" role="form">
                <textarea name="tweet" cols="50" rows="5" class="form-control" placeholder="例：Hello World!"><?php echo $detail_table['tweet']; ?></textarea>
                <input type="hidden" name="tweet_id" value="<?php echo $detail_table['tweet_id']; ?>">
